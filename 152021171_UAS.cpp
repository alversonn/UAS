...
int main()
{
  ...
  for (int i = 0; i < banyak; i++)
  {
    ...
    while (dta[i].data_y <= 0) {
            cout << "Nilai y tidak boleh kurang dari 0. Masukkan nilai x: ";
            cin >> dta[i].data_y;
    }
  ...
  bagAtas = (banyak * sigma_xy) - (sigma_x * sigma_y);
  bagBawahKiri = sqrt((banyak * sigma_x2) - (pow(sigma_x, 2)));
  bagBawahKanan = sqrt((banyak * sigma_y2) - (pow(sigma_y, 2)));
